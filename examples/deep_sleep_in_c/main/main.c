...
void app_main(void)
{
    ...
    // Función para generar coordenadas aleatorias dentro de los límites de Buenos Aires
    void generateRandomLocation(float *latitude, float *longitude) {
        ...
        // Genera latitud y longitud aleatoriamente dentro de los límites especificados
    ...
    snprintf(message, sizeof(message), "{\"value\":{\"lat\":%.4f,\"lon\":%.4f}}", latitude, longitude);
    ttn_response_code_t res = ttn_transmit_message((uint8_t *)message, strlen(message), 1, false);
    // Imprime los valores generados y el mensaje
    printf("Latitude: %.4f\n", latitude);
    printf("Longitude: %.4f\n", longitude);
    printf("Message: %s\n", message);
    // Verifica el resultado del envío a TTN
